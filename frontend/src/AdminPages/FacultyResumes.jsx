...
const FacultyResume = () => {
  ...
  useEffect(() => {
    const fetchFacultyResumes = async () => {
      try {
        setLoading(true);
        ...
        const { resume: facultyResumes, total, pages } = response.data.data;

        console.log("API Response Data:", facultyResumes); // Debugging
        setFacultyResumes(facultyResumes || []);
        setTotalPages(pages);
        setLoading(false);
      } catch (error) {
        setLoading(false);
      ...
      <div className="bg-gradient-to-r from-purple-600 to-purple-900 rounded-2xl mt-2 w-full max-w-screen-xl mx-auto py-10 px-5 sm:px-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {facultyResumes.length > 0 ? (
            facultyResumes.map((resume) => (
              <div
                key={resume._id}
                className="bg-white p-4 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300"
              >
                <div className="mb-4">
                  <h3 className="font-bold text-lg text-purple-700">{resume.name}</h3>
                  <div className="text-sm text-gray-500">
                    <div className="text-base font-medium text-black">Experience</div>
                    {resume.experience}
                  </div>
                  <div className="text-sm text-gray-500">
                    <div className="text-base font-medium text-black">Publications</div>
                    {resume.publications || "N/A"}
                  </div>
                  <div className="text-sm text-gray-500">
                    <div className="text-base font-medium text-black">Research Grants</div>
                    {resume.research_grants || "N/A"}
                  </div>

                  <div className="text-sm text-gray-500">
                    <div className="text-base font-medium text-black">Professional Presentation Selected</div>
                    {resume.selected_professional_presentation || "N/A"}
                  </div>

                  <div className="text-sm text-gray-500">
                    <div className="text-base font-medium text-black">Brief Statment On Research</div>
                    {resume.brief_statement_of_research || "N/A"}
                  </div>

                  <div className="text-sm text-gray-500">
                    <div className="text-base font-medium text-black">Service</div>
                    {resume.service || "N/A"}
                  </div>

                  <div className="text-sm text-gray-500">
                    <div className="text-base font-medium text-black">Memberships</div>
                    {resume.memberships || "N/A"}
                  </div>
                </div>

                <div className="flex justify-between items-center">
                  <p className="text-xs text-gray-400">
                    {moment(resume.createdAt).format("YYYY-MM-DD HH:mm:ss")}
                  </p>
                  <p className="text-xs text-gray-400">
                    {moment(resume.updatedAt).format("YYYY-MM-DD HH:mm:ss")}
                  </p>
                </div>
